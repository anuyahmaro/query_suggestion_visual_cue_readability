...
class dbadapter
{
    ...
    function __construct(){
        $this-> conn = mysqli_connect("localhost:8889","root","root","my_computing_artifact");
...
//				$islike = $s['islike'] == 'true'?1:0;
                ...
                $prefer = $s['prefer'];
				 ...
				 $query= "insert into `suggestions`
							(`sessionid`,`queryid`,`suggestion`,`source`,`dateadded`,`complexity`, `user_selected`) values
							('$sessionid','$id','$suggestion','$src','$now','$complexity', '$prefer'
							)";
								$this->execute ($query);
